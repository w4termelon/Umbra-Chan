...
namespace Umbra_C.Modules
{
    ...
    public class BasicModule : InteractionModuleBase<SocketInteractionContext>
    {
        ...
        public BasicModule(InteractionHandler handler, ApplicationDbContext db)
        {
            _handler = handler;
            _db = db;
        }
        ...
        [SlashCommand("admin", "Füge ein Bot-Admin hinzu.")]
        public async Task AddAdminAsync(IUser user)
        {
            var dataUser = _db.Users.Find(user.Id);
            if(dataUser == null)  await RespondAsync(text: $"{user.Username} wurde nicht gefunden!", ephemeral: true);
            dataUser.IsAdmin = true;
            var changes = _db.SaveChangesAsync();
            await RespondAsync(text: $"{user.Username} wurde als Admin hinzugefügt!", ephemeral: true);
            await changes;
        }
